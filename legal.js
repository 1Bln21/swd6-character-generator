...
function docPrivacy(d) {
  ...
  if (LANG === 'en') return `
    ...
    <p>Sent in both cases: the error message or your text, the place in the program code, the page, the version of the application, the browser family with its major version (for example “Firefox 128 / Windows”) and the set language. An automatic error message is technical text generated by the browser; it can occasionally quote a value that was on the page at that moment. <b>No IP address is stored, no complete user agent, no identifier that would follow you from one visit to the next.</b> If you are signed in, your user name is attached so that an answer by ticket is possible; without an account the report is anonymous. A character sheet is only sent along if you tick that box in the dialog yourself, and pictures are removed from it beforehand.</p>
    ...
    <p>Characters are stored until you delete them. Your account and all associated data will be deleted on request to ${esc(d.email)}. This includes the rounds you lead with everything at their table top, and in other people's rounds your tokens, your log entries, your call data and the maps and music you uploaded there as a co-game master; those rounds themselves remain. Bug reports you wrote yourself are deleted. Automatic error reports are kept, without your user name and without an attached sheet; since an error message can occasionally quote a value from the page (section 8), they cannot be guaranteed to contain nothing about you.</p>
    <h4>10. Encryption</h4>
  ...
  return `
    ...
    <p>Übertragen werden in beiden Fällen: die Fehlermeldung bzw. dein Text, die Stelle im Programmcode, die Seite, die Version der Anwendung, die Browser-Familie mit Hauptversion (z. B. „Firefox 128 / Windows“) und die eingestellte Sprache. Eine automatische Fehlermeldung ist ein vom Browser erzeugter technischer Text; sie kann im Einzelfall einen Wert wiedergeben, der in dem Moment auf der Seite stand. <b>Es wird keine IP-Adresse gespeichert, kein vollständiger User-Agent und keine Kennung, die dich über Besuche hinweg wiedererkennbar macht.</b> Bist du angemeldet, wird dein Benutzername mitgeschickt, damit eine Antwort per Ticket möglich ist; ohne Konto ist die Meldung anonym. Ein Charakterbogen wird nur mitgesendet, wenn du das im Dialog selbst ankreuzt – Bilder werden vorher entfernt.</p>
