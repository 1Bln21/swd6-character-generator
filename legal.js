...
function docPrivacy(d) {
  ...
  if (LANG === 'en') return `
    ...
    <p>If the game master plays <b>music from YouTube</b>, your browser loads YouTube's embedded player from youtube.com – only then, not before. Google (USA) receives your IP address and can set cookies; Google's privacy policy applies. Music uploaded to the round plays from this server instead.</p>
    <p>Legal basis is Art. 6 (1) (b) GDPR (providing the function you use). If the game master deletes the round, its maps, tokens, log and music are deleted with it, including the files, unless another round uses the same file.</p>
    ...
    <p>Characters are stored until you delete them. Your account and all associated data will be deleted on request to ${esc(d.email)}. This includes the rounds you lead with everything at their table top, and in other people's rounds your tokens, your log entries and your call data; those rounds themselves remain. Bug reports you wrote yourself are deleted; automatic error reports contain nothing you typed and are kept without any link to you.</p>
    <h4>10. Encryption</h4>
    <p>This site uses HTTPS (TLS) to protect the transmission of your data.</p>
    <h4>11. Your rights</h4>
    <p>You have the right to information (Art. 15 GDPR), rectification (Art. 16), erasure (Art. 17), restriction of processing (Art. 18), data portability (Art. 20) and to object (Art. 21). You also have the right to lodge a complaint with a supervisory authority (Art. 77 GDPR). If you have an account, you can download a copy of the data stored about you at any time in the Online window (“Download my data”).</p>
  ...
  return `
    ...
    <p>Spielt die Spielleitung <b>Musik von YouTube</b> ab, lädt dein Browser den eingebetteten Player von youtube.com – erst dann, nicht vorher. Google (USA) erhält dabei deine IP-Adresse und kann Cookies setzen; es gilt die Datenschutzerklärung von Google. In die Runde hochgeladene Musik läuft dagegen von diesem Server.</p>
    <p>Rechtsgrundlage ist Art. 6 Abs. 1 lit. b DSGVO (Bereitstellung der Funktion, die du nutzt). Löscht die Spielleitung die Runde, werden Karten, Marken, Protokoll und Musik mitgelöscht, einschließlich der Dateien, sofern keine andere Runde dieselbe Datei nutzt.</p>
